...
| **S5** | **The text writer, R6 splicing, loop C.** Both forms | **Writer done**, both forms, 10 round-trip tests; `package`/`manifest`/`esc`/`VERSION` extracted to `grind-core` now that a second caller pulls on them, with the sheet's R6 figures unmoved as proof it was a move. **R6 splicing and loop C are still outstanding** — this writer regenerates | large |
