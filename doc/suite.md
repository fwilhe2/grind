...
| **S4** | **The text model and reader.** `loc.rs`, `Block`/`Run`, block ids, contexts over the existing driver | **Model, addressing and reader done** — 28 tests, and `implemented()` checked against `doc/text-core.md` by `text/tests/scope.rs`. **Loop A over Writer's corpus is still outstanding**, and with it the `GRIND_LO_CORPUS` move to the checkout root | large |
