...
`generate-rpm` needs its `requires` block spelled out because it has no `$auto`. **Built that
way, as `suite/`** — with one surprise: `cargo deb` takes the architecture from the build target
and has no key for `all`, so the `.deb` says `amd64` (harmless, since the four it depends on come
from the same job) while the `.rpm` is `noarch` through `generate-rpm -a`.
...
| **C** — round-trip differential | green both directions | **Done, green both directions.** 14 documents out (7 cases x 2 forms), 20 corpus documents and 5095 blocks back, 0 differences. One documented loosening — style names, because this writer declares no styles (`doc/odt-format.md` §5b). Gates CI, against the same pinned container as the spreadsheet's: that image was Calc-only when this was written, so the tests probe the oracle's capability rather than assuming it, and started gating the day it was rebuilt with Writer — no file changed |
...
## The layout fork — **reopened; see `doc/text-layout.md`**

**This was the product decision this document could not make, and everything from S7 onward
depends on it.** Both branches below are real; they lead to different programs.

It was closed on Path A at S7 and **reopened immediately**, on an objection this section did
not weigh: `CLAUDE.md`'s architecture rule puts all logic in the core, and line layout is not
rendering — Down-arrow, Home/End, click-to-caret and selection extents are all defined in terms
of a *line*, so Path A puts a piece of the editing model in three shells that will disagree.
**`doc/text-layout.md` is now the normative document for this decision**, it identifies a third
path this section missed (line layout in the core with metrics injected, pagination still
gated), and it recommends that one. Everything below is kept as the record of the argument
...
| **S11** | **Package the suite.** Meta-package, the container, README rewritten as a suite pitch | `artifacts.yml` produces all five packages; `reuse lint` green; `doc/shell-matrix.md` full and green (R10); double-clicking a `.fodt` opens the text app on a clean machine | **Done (2026-09-21).** The meta-package is `suite/`, the stub crate this document recommended, and `cli/tests/packaging.rs` holds it to the four. All five `.deb`s were built in release and installed through it with `apt`, and `gio open note.fodt` started the installed text app, with all four media types resolving to the right window both beside LibreOffice and in a data directory holding only grind's entries. The R10 matrix is `doc/feature-matrix.md`, which grew into what `doc/shell-matrix.md` was to be; `reuse lint` gates in CI |
