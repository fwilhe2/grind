...
double-click or OK jumping straight to its block. There is still no way to reach a level past 3
or a list item's depth from this window at all — a heading below level 3 or a list has to be
made one from `grind text kind` in the meantime. The clipboard carries
**plain text only** — one `\r\n` per block, no
