...
block — no selection needed, unlike the three toggles above. There is still no **outline** dialog
listing the headings to jump between rather than typing one's address, and no way to reach a level
past 3 or a list item's depth from this window at all. The clipboard carries
**plain text only** — one `\r\n` per block, no
`CF_RTF` and no `HTML Format` — so formatting does not survive a copy into WordPad. And the
**menus mostly still don't know which pane they are over**: Recalculate and the four sheet verbs
are in the bar while a text document is open and do nothing when chosen, and the new Format menu's
three verbs are the same no-op on the grid — `MF_GRAYED` means tracking menu state and W7 is where
the menus are finished.
