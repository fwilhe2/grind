...
strip** — reading and writing `char_style`/`set_char_style` from a toolbar over the selection is
unbuilt — but three of its four toggles are reachable a different way first: **Ctrl+B/I/U and a
Format menu**, over the same toggle logic `ui_tui::emphasise_selection` uses (on across the whole
selection, or off when it already agrees), so bold, italic and underline are things this window
can *do* before they are things it draws a button for. **Go-to has since landed too**, as F5 or
...
with no window at all — of which verb means anything for which document kind, and `build_menu`
turns a `false` answer into `EnableMenuItem`'s `MF_GRAYED` rather than a silently ignored click.
Recalculate and the four sheet verbs grey out over a text document and the Format menu's eight
grey out over a spreadsheet, rebuilt whenever `adopt` changes which kind the window holds — File ▸
Open on the other document type, or File ▸ New. What W7 still owes is state *inside* one kind —
