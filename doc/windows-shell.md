...
now understands. There is still no way to change a **block's kind** and no **outline** dialog
listing the headings to jump between rather than typing one's address. The clipboard carries
**plain text only** — one `\r\n` per block, no
