...
| **A table's own style, and its formulas** | Column widths when a document needs them; the formula question when §5 answers it | Tables themselves are **built** now (`doc/text-core.md`), read, written, projected and drawn: a block carries the coordinate of the cell it is in, and a cell holds *blocks* exactly as the body does (rng:16126). What is not carried is the table's `table:style-name` — its column widths, borders and background — so a shell lays a table out in equal columns and a regenerate loses the widths (R6 keeps them for a table nobody edited). `table:formula` in a cell stays out for a harder reason: `doc/odt-format.md` §5's first `UNVERIFIED` question is whether a Writer table's formula is OpenFormula at all, and nothing may be built on an unanswered one |
| **A table nested inside a table cell** | A document that needs one | Read, and *flattened*: the inner table's paragraphs become paragraphs of the cell holding it, so the text is kept and the inner structure is not. The alternative was worse — a nested table splits the outer table's run of blocks in two, and a flat model cannot then say which half is which (`text/src/model.rs`, `Cell`) |
| **Merging cells from a UI** | A shell with a table toolbar | The model carries `table:number-columns-spanned` and `table:number-rows-spanned`, reads them, writes them back with the `table:covered-table-cell` positions they imply, projects them as `span=`/`rows=` and *draws* them merged. Nothing **creates** one: `grind text table` makes a plain grid, and so does every shell |
