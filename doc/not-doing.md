...
| **Footnotes, fields, a table of contents** | Somewhere to put one | A footnote's *content* models fine; its *placement* is the page model, which Path A does not have. A continuous view can still show one — at the end of the flow, or inline — so this is a shell question rather than a blocked one, and it waits for a shell |
...
| **Pagination — a page model, and everything that needs one** | **Loop D at a stated floor**, and a real week of use proving the continuous view insufficient | The layout fork, and it is now **decided rather than open**: `doc/suite.md` S7 took Path A. Page size, margins, `fo:break-before`, headers, footers and footnote placement are read, preserved and round-tripped, and never rendered as pages. Reopening it means building the engine *and* the differential that keeps it honest — render a corpus document our way and LibreOffice's way, compare line-break positions and page boundaries, ratchet the percentage exactly as loop B does. The pinned oracle pays off twice there, because a digest pins the *fonts* as well as the renderer and an unpinned layout differential is noise. Until then the honest label is in `README.md`, in this document's voice |
