...
`PIE_SAMPLES_PER_TURN` fine enough that the seam does not show). Each series (bar, line) or
category (pie) gets [`grind_sheet::series_color`] in the same order the writer assigns it, so
what is on screen matches what gets saved. No title, no legend, no axis ticks — the same scope
line as the format itself, drawn rather than written.
...
**Not built**: no dialog to add or remove a chart from the GUI (`grind sheet chart-add`/
`chart-remove` are the only way in this build), no visual feedback beyond the accent outline and
handle already used for a resize, and no keyboard-driven repositioning — a mouse is what
"dragged" means here.
