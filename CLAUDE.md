...
| `doc/xlsx-import.md` | **Phase 11 — reading `.xlsx`, built through X5 (values, formulas, number formats, styles and geometry, and the document level).** Normative for that phase: its own crate behind a feature flag, one way in and never out, a fidelity report that is part of the output, and "Transitional, and the real world" — the section that decides whether the filter opens other people's files, since Transitional is what every real producer writes and markup compatibility is the tax they charge. **ECMA-376 is the source; LibreOffice stays the oracle** |
