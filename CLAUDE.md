...
`grind-win32` is the Windows shell (`doc/windows-shell.md`), built **through W3**: a window, the
...
carried four references with it — whose every sentence is a pure function in `notice.rs`. No
clipboard and no text pane yet (W4 and W5). What is unusual about it is that the whole thing is
examinable from Linux, because `cargo check` does not link:
...
an empty string was an access violation reading address 2.
