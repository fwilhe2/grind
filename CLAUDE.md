...
| `doc/xlsx-import.md` | **Phase 11 — reading `.xlsx`, built through X4 (values, formulas, number formats, styles and geometry).** Normative for that phase: its own crate behind a feature flag, one way in and never out, a fidelity report that is part of the output, and "Transitional, and the real world" — the section that decides whether the filter opens other people's files, since Transitional is what every real producer writes and markup compatibility is the tax they charge. **ECMA-376 is the source; LibreOffice stays the oracle** |
...
Since the build is X4, every cell's value and kind is asserted — one claim per cell — and so is
...
in both directions**: `PENDING` (11 — a claim that starts passing *fails the test*, so the entry
must be deleted when its milestone lands; X1 deleted three, X2 five and X4 two),
...
asked for). 1697 of 1735 claims hold: 1340 of 1341 cells,
122 of 125 formulas and 89 of 107 displays. X4's expectations are in the manifest's *prose*
