...
| `doc/xlsx-import.md` | **Phase 11 — reading `.xlsx`, built through X3 (values, formulas and number formats).** Normative for that phase: its own crate behind a feature flag, one way in and never out, a fidelity report that is part of the output, and "Transitional, and the real world" — the section that decides whether the filter opens other people's files, since Transitional is what every real producer writes and markup compatibility is the tax they charge. **ECMA-376 is the source; LibreOffice stays the oracle** |
...
Since the build is X3, every cell's value and kind is asserted — one claim per cell — and so is
...
in both directions**: `PENDING` (13 — a claim that starts passing *fails the test*, so the entry
must be deleted when its milestone lands; X1 deleted three and X2 deleted five),
...
`DECIDED_OTHERWISE` (7 — two refusals this filter deliberately does not make, both in
`hostile/`, three manifests that disagree with their own fixture's bytes, and two spellings
where ours is as true as the one asked for). 1696 of 1734 claims hold: 1340 of 1341 cells,
122 of 125 formulas and 89 of 107 displays. `fixtures.rs` keeps its own job:
...
elapsed time) — every other loss being a cosmetic piece the format was carried without.
...
each cell **displays** as well, wherever both sides put a format on it.
