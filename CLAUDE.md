...
| `doc/xlsx-import.md` | **Phase 11 — reading `.xlsx`, built through X2 (values and formulas).** Normative for that phase: its own crate behind a feature flag, one way in and never out, a fidelity report that is part of the output, and "Transitional, and the real world" — the section that decides whether the filter opens other people's files, since Transitional is what every real producer writes and markup compatibility is the tax they charge. **ECMA-376 is the source; LibreOffice stays the oracle** |
...
Since the build is X2, every cell's value and kind is asserted — one claim per cell — and so is
...
than merely describing them. The gap between claim and capability lives in **two tables checked
in both directions**: `PENDING` (13 — a claim that starts passing *fails the test*, so the entry
must be deleted when its milestone lands; X1 deleted three and X2 deleted five) and
...
where ours is as true as the one asked for). 1607 of 1627 claims hold, 1340 of 1341 cells and
122 of 125 formulas. `fixtures.rs` keeps its own job:
